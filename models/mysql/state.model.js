    ...
    return {
        ...
        addState : function(stateId, stateName, stateFoundationDate){
            return new Promise((resolve, reject) => {
                ...
                dbcon.query(query, [stateId, stateName, stateFoundationDate], (err, data) => {
                    if ( !err ) {
                        resolve(data);
                    } else {
                        reject(err);
        ...
        editStateById : function(stateId, stateName, stateFoundationDate, id) {
            return new Promise((resolve, reject) => {
                let query = 'UPDATE STATE SET DR_IDENTIFIKATOR = ?, DR_NAZIV = ?, DR_DATUM_OSNIVANJA = ? WHERE DR_IDENTIFIKATOR = ?;';
                dbcon.query(query, [stateId, stateName, stateFoundationDate, id], (err, data) => {
                    if ( !err ) {
                        resolve(data);
                    } else {
                        reject(err);
